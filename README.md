...
### 抑制也有代价：一处「压过头」的漏报（v1.5.2 修复）

「被更新的整理记录接管」（`superseded`）原本的判据是「**同剧同季**有 id 更大的记录」。
但 `transferhistory` 是**按文件逐条**记录的：一部剧一季 40 集就有 40 条记录、各自 `dest`
不同。于是「同剧同季只认最新那条」等于把这一季**除最新一集外的所有记录**都判成
`superseded` —— 而 L1 在 `superseded` 为真时会把「硬链接断开」降级成提示，
**这些剧集的断链检查形同虚设**。

实测：1625 条记录被这条规则压制，其中 1620 条目标文件存在且 `mode=link`。
（当时真去比对 inode，被压制的断链数是 0 —— 没有造成实际漏报，但纯属运气。）
...
同一条修复还解决了「接管者自己也成了僵尸」引起的**级联剥皮**：
原实现只看 id 大小，僵尸记录要等接管者先被清掉才浮出来，清理变成每轮只剥一条
（实测同一部剧 5 条记录剥了 5 轮；而某剧单季有 122 条记录，最坏要 122 轮）。
现在只有「接管记录的目标文件确实还在」才算真接管，同批孤儿一轮内一起清。
查不到接管记录时按「算数」保守处理 —— 宁可不删，不可错删。
...
| **异常（notify）** | **0 条** |
| 非异常提示 | 984 条 |
| 其中被覆盖跳过（superseded） | 234 条 |
| 未挂载跳过 | 0 条 |
| 深度复核（TMDB） | 7 条 |
| 累计清理僵尸整理记录 | 57 条 |

清理轮次是一个**逐轮递减并归零**的过程（23 → 16 → 8 → 6 → 1 → 3），
不是异常。上界可证：同一个死目标路径下最多 N 条记录，每轮至少清掉一条，
N 轮后该路径必为空 —— 绝不会无限删。
...
- **「自动清理僵尸记录」的清理可能需要跑不止一轮。** 一批记录里可能存在「旧记录被新记录
  接管」的关系链；v1.5.2 起，只有当接管记录**自身的目标文件还在**时才构成接管，
  因此同一批孤儿通常一轮内就能一起清掉（更早的版本会每轮只剥一条）。
  若一轮没清完，跑第二轮即可 —— 每轮至少清掉一条，收敛有上界，不会无限循环。
